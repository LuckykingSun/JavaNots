...
- 对象的多态（核心 ，困难，重点）
  ...
  - 运行类型是可以变化的，可以通过getClass（）来查看运行的类型
...
3. super  的访问不限于直接父类，如果爷爷类和本类中有同名的成员，也可以使super去访问爷爷类的成员，如果多个基类（上级类） 中都有同名的成员。使用super访问遵循就近原则  A--> B --->C ，当然也要遵守访问权限的相关规则
...
| ------ | ---------------------------------------- | ------------------------------------ | ------------------------------------------------------------ |
| ==     | 比较运算符                               | 用于基本数据类型，可以判断值是否相等 | 用于引用类型，判断留两个对象是否相等                         |
| equals | object类的方法啊，java类都可以说明equals | 不可以                               | 可以，默认是判断两个值是否相等，但是子类往往重写该方法，比如String，integer |
...
![image-20230717201207142](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230717201207142.png)

## 房屋出租系统

实现基于文本页面的《房屋出租的软件》

能够实现对房屋信息的添加，删除和修改（但是需要用数组来解决实现），能够打印房屋明细表



分析编写的框架（分层模式）

![image-20230718144539021](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230718144539021.png)
...
可以看到static实例是保存在class实例的尾部

Class对象确实在堆中

static 变量是对象共享，不管static 变量在哪里，有两个共识
...
类变量也叫静态变量 / 静态属性 。是该类的所有对象共享的变量，任何一个该类的对象去访问它时，取到的都是相同的值，同样一个该类对象去修改它时，修改的也是同一个变量

#### 如何定义类变量

定义语法：

访问修饰符 static 数据类型 变量名 【推荐的，比较规范】
...
或者 对象名 类变量名【静态变量的修饰符的访问权限和范围 和 普通属性是一样的】
...
1. 什么时候需要用类变量
   ...
   当需要让某个类的所有对象都共享一个变量时，就可以考虑使用类变量（静态变量）；比如 定义学生类

2. 类变量与实例变量（普通属性）的区别

   类变量是该类的所有对象共享的。而实力变量是每个对象独享的

3. 加上static 称为类变量或者静态变量，否则称为实例变量/ 普通变量/非静态变量

4. 类变量是可以通过 类名 类变量名 或者 对象名。类变量名 来访问

   但是java设计者推荐我们使用 类名.类变量名方式访问【前提是 满足访问修饰符的访问权限和范围】

5. 实例变量不能通过类名 ，类变量名 方式访问

6. 类变量在类加载的时候就开始初始化来了，也就是说，即使你没有创建对象，只要类加载了，就可以使用类变量了
...
使用方式： 类名.类方法名 或者 对象名.类方法名【前提是 满足访问修饰符 的访问权限和范围】
...
class Stu{
    ...
    //说明：1.当方法使用了static修饰后，该方法就是静态方法
    //      2.静态方法就可以访问静态的属性 /变量
...
1. 类方法和普通方法都是随着类的加载而加载，将结构信息储存在那方法区：
...
4. 类方法中不允许使用 和 对象有关的关键字 ，比如 this 和 super ，普通方法（成员方法）可以
...
![image-20230720091913660](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230720091913660.png)

![image-20230720092345590](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230720092345590.png)

静态方法只能访问静态成员，非静态方法，可以访问所有的成员，编写代码的时候，仍然要遵守访问权限规则
...
- java虚拟机在执行main方法的时候不必创建对象，所以该方法必须是static

- 该方法接收String类型的数组参数，该数组中执行java 命令时传递给所运行的类的参数

- java执行的程序是 参数1，参数2，参数3

  ![image-20230720094435648](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230720094435648.png)

#### 特别提示

- 在main方法中，我们可以直接调用main 方法所在类的静态方法 或者静态属性
- 但是，不能直接访问该类中的非静态成员，必须创建该类的一个实例对象后，才能通过这个对象去访问该类中的非静态成员
...
但是和方法不同，没有方法名，没有返回，没有参数，只有方法体，而且不用通过对象或者类显式调用，而是加载类时，或者创建对象时隐式调用
...
- 代码块分为两类 ，使用static 修饰的 叫静态代码块，没有static 修饰的 ，叫做普通代码块
- 逻辑语句可以分为任何逻辑语句（输入，输出，方法调用，循环，判断等）
- ;号可以写上，也可以省略

### 注意事项和使用细节

1. static 代码块也叫静态代码块，作用就是对类进行初始化，而且它随着类的加载而执行，并且只会执行一次，如果是普通代码块，每创建一个对象，就执行，类加载和他没有p点关系
...
- 使用类的静态成员时（静态属性，静态方法）
  ...
  eg：A类 extends B类 的静态块

3. 普通的代码块，在创建对象实例的时候，会被隐式的调用，被创建一次就会调用一次，如果只是使用类的静态成员时，普通代码块不会执行 
...
5. 构造器  的最前面其实隐含了super（）和调用普通代码块，新写一个类演示，静态相关的代码块，属性初始化，在类加载时，就加载时，就执行完毕，因此是优先于 构造器和普通代码块执行的
...
1. 所谓类的单例设计模式，就是采取一定的方法保证在整个的软件系统中，对某个类只能存在一个对象实例，并且该类只提供一个取得其对象实例的办法
...
1. 两者最主要的区别在于创建对象的时机不同，饿汉式是在类加载的就创建了对象实例，而懒汉式是在使用时才创建
2. 饿汉式不存在线程安全问题，懒汉式存在线程安全问题
3. 饿汉式存在着资源浪费的可能，因为如果程序员一个对象实例都没有使用，那么饿汉式创建的对象就浪费了，懒汉式是使用时才创建，就不存在这个问题
4. 在我们javase标准类中，java.lang.Runtime就是经典的单例模式

饿汉式问题，在类加载的时候就开始创建，可能存在资源浪费
...
1. final修饰的属性又叫常量，一般用xx  xx xx 来命名
2. final修饰的属性在定义时，必须赋初值，并且以后不能再修改，赋值可以在如下位置之一【选择一个位置赋初始值即可】
   - 定义时， 如`public final double TAX_RATE = 0.08;`
   - 在构造器中
   - 在代码块中
3. 如果final 修饰的 属性是静态的 ，则初始化的位置只能是
   1. 定义 时
   2. 在静态代码块 不能在构造器中赋值
4. final类不能继承，但是可以实例化对象
5. 如果类不是final类，但含有final方法，该方法虽然不能重写，但是可以被继承
6. 一般来说，如果一个类已经是final 类了，就没有必要再将方法修饰成final方法
7. final 不能修饰构造方法（构造器）
8. final 和 static 往往搭配使用，效率更高，不会导致类加载，底层编译器做了优化处理
9. 包装类`（Integer ，Double ， Float  ，Boolean 等都是 final`），String也是 final类
...
4. 抽象类，是考官爱问的知识点，在框架和设计模式使用较多

### 注意事项和使用细节

1. 抽象类不能被实例化

   1. 抽象类不一定要包含abstract 方法，也就是说，抽象类可以没有`abstract` 方法，还可以有实现的方法

2.  一旦类包含了`abstract`方法，则这个类必须声明为`abstract`

3. `abstract` 只能修饰类和方法 ，不能修饰属性和其他的

4. 抽象类可以有任意的成员【抽象类的本质还是类】，比如，非抽象方法 ，构造器 ，静态属性等等

5. 抽象方法不能有主体，即不能实现

6. 如果一个类继承了抽象类，则它必须实现抽象类的所有抽象方法：除非它自己也声明为abstract 类
...
![image-20230721152116301](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230721152116301.png)
...
接口就是给出一些没有实现的方法，封装到一起，到某个类使用的时候，在根据具体情况把这些方法写出来   语法：
...
**小结：** 1.在JDK 7.0之前，接口里的所有方法都没有方法体

			2. 在JDK 8.0后 接口类可以有静态方法，默认方法，也就是说接口可以有方法的具体实现

### 注意事项和使用细节

1. 接口不能被实例化

2. 接口中的所有的方法是 `public`方法 ，接口中抽象方法，可以不用abstract 修饰 图示
...
3. 一个普通类实现接口，就必须将该接口的所有方法都实现 ，可以用 alt + enter 来解决

4. 抽象类实现接口，可以不用实现接口的方法

5. 一个类同时可以实现多个接口【举例】

6. 接口中的属性，只能是final 的，而且是public static final 修饰符，比如: `int a = 1` ;实际上是`public static final int a = 1`；必须初始化
...
9. 接口的修饰符 只能是 public 和默认 ，这点和类的修饰符是一样的
   ...
   ![image-20230722111021289](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230722111021289.png)

### 实现接口   vs    继承类

- 接口和继承的解决的问题不同
  - 继承的价值在于 ： 解决代码的复用性 和 可维护性
  - 接口的价值在于 ： 设计，设计好各种规范（方法），让其他类去实现这些方法
-  接口比继承更的灵活

接口比继承更加的灵活，继承是满足 is - a 的关系 ，而接口只需要满足  like a 的关系
...
1. 多态参数
   ...
   前面的Usb接口案例，既可以接受手机对象，又可以接受相机的对象，就体现了接口的多态（接口的引用 可以指向 实现了接口的类的对象）

2. 多态数组

   演示案例 ，给USb数组中，存放Phone 和相机对象 ，Phone 类还有一个特有的 方法call（） ，请遍历Usb数组，如果是 Phone 对象，除了调用Usb接口的定义的方法外，还需要调用Phone 特有的方法call

   ![image-20230722151103443](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230722151103443.png)

3. 接口存在 多态传递的现象

![image-20230722160315980](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230722160315980.png)
...
一个类的内部又完整的嵌套了另一个类结构，被嵌套的类称之为内部类（`inner Class`），嵌套其他的类称之为外部类（`outer Class`），是我们类的第五大成员【类的五大成员包含  ； 属性 ，方法， 构造器，代码块，内部类】，内部类的最大特点就是可以直接访问私有的属性，并且可以体现为类与类之间的包含关系
...
说明：局部内部类是定义在外部类的局部位置，，通常在方法，比如方法中，并且有类名

- 可以直接访问外部类的所有成员，包含私有的

- 不能添加访问修饰符，因为它 的地位就是一个局部变量，局部变量是不能使用修饰符的，但是可以使用 final 修饰，因为局部变量也可以使用final 
...
- 外部其他类-----不能访问------>局部内部类（因为 局部内部类是一个局部变量）

- 如果外部类和局部内部类的成员重名时，默认遵循就近原则，如果想访问外部类的成员，则可以使用（外部成员this.成员）去访问
...
2. 匿名内部类的语法比较的奇特因为内部类既是一个类的定义，同时它本身也是一个对象，因此从语法上来看，它既有定义类的特征，也有创建对象的特征，对前面的代码分析可以看出这个特点
...
2. 可以添加任意访问修饰符（public peotected  默认 private），因为它的地位就是一个成员
3. 作用域  ：和外部类的成员一样，为整个类体，在外部类的成员方法中创建成员内部类对象，再调用方法
4. 成员内部类---->访问---->外部类成员 （比如：属性）【访问方式：直接访问】）
5. 外部类----访问----->成员内部类（{访问方式 ： 创建对象，再访问}）
6. 外部其他类---->访问----->成员内部类

![image-20230724160009536](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230724160009536.png)

#### 静态内部类的使用

说明：静态内部类是定义在外部类的成员位置，并且有static 修饰

1. 可以直接访问外部类的所有静态成员，并且有static 修饰
...
6. 外部其他类----访问----->静态内部类
   ...
   ```
   //方式1，通过一个方法来获取一个静态内部类的对象实例
   Inner02 inner02 = Outer02.getInner02();
   Inner02.say();
   ```

   ![image-20230724162956626](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230724162956626.png)

   ```java
   public static void main(String[] args){
   //两种方式
   	B02 b02_1 = new A03.B02()；//这里就不会创建一个A03实例
   	b02_1.say();
   	
   	B02 b02_2 = new A03().getB02Object();//第二种方式
   }
   ```

7. 如果外部类和静态内部类的成员重名时，静态内部类访问时，默认遵循就近原则，如果想访问外部类的成员，则可以使用（外部类名.成员）去访问

​	![image-20230724163301532](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230724163301532.png)
...
![image-20230724164306721](C:\Users\LUCKYKING SUN\AppData\Roaming\Typora\typora-user-images\image-20230724164306721.png)
...
枚举对应的英文（enumeration ，简写enum）

枚举是一组常量的集合

枚举属于一个特殊的类，里面只包含一种有限的特定的对象
...
- 传统的public static final Season2 SPRING = new Season2（“春天”，“温暖”）；简化成了SPRING（“春天“，”温暖“），这里必须要知道他调用的是哪个构造器
...
- toString：Enum类已经重写过了，返回的是当前对名，用于返回对象的属性信息
- name : 返回当前对象名（常量名），子类中不能重写
- ordinal ：返回当前对象的位置号，默认从0开始
- values：返回当前枚举类中所有的常量
- valueOf：将字符串转换成枚举对象，要求字符串必须为已有的常量名，否则报异常
- compare To：比较两个枚举常量，比较的就是位置号

##### enum 常用方法说明

说明：使用关键字enum 时，会隐式继承Enum类，这样我们就可以使用Enum 类相关的方法
...
- 注解（Annotation）也被称之为元数据（Metadata），用于修饰解释 包·类·方法·属性·构造器·局部变量等数据信息
- 和注释一样，注意不影响程序逻辑，但注解可以被编译或运行，相当于嵌入在代码中的补充信息
- 在javaSe中，注解 的使用目的比较的简单，例如标记过时的功能，忽略警告等，在JavaEE中注释占据了更重要的角色，例如用来配置应用程序的任何切面，代替java EE 旧版中所遗留的繁冗代码和 XML配置等
...
基本的Annotation 时要在其前面添加 @ 符号，并把该Annotation当成一个修饰符来使用，用于修饰它支持的程序元素

### @override 

限定某个方法，是重写父类的方法，该注释只能用于方法
...
@interface 的说明  @interface 不是interface，是注解类，是JDK5.0 之后加入的

#### override 使用说明

- @Override 表示指定重写父类的方法（从编译层面验证），如果父类没有fly方法，则会报错
- 如果不写@Override 注解，而父类仍有public void fly(){} ,仍然构成重写
- @overload 只能修饰方法，不能修饰其他类，包，属性等等
- 查看@overload 注解源码为 @Target（ElementType.METHOD），说明只能修饰方法
- @Target 是注解的注解，称为元注解
...
4. @Deprecated 的作用是 可以做到新旧版本的兼容和过度
...
- rawtypes 是忽略没有指定 泛型 的警告（传参时没有指定的警告错误）
- unuesd是忽略没有使用某个变量的警告错误
...
元注解：本身的作用不大，但是源码时，要知道这是干什么滴

元注解的种类

- Retention //指定注释的作用范围
- Target  // 指定注解可以在哪些地方使用
- Documented //指定该注解是否会在 javadoc体现
- Inherited //子类会继承父类注解
